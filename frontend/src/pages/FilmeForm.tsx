...
import {CAMPOS_VAZIOS, type CamposFilme, maskData, validarFilme} from './filme/validarFilme';
...
function Acoes({modo, desabilitado}: Readonly<{ modo: 'novo' | 'editar'; desabilitado: boolean }>) {
    return (
        <div className="mt-9.5 flex flex-wrap items-center justify-center gap-3.5">
            ...
            <button type="submit" className="btn btn-primary btn-tall min-w-[min(340px,100%)] px-11.5"
...
export function FilmeForm() {
    ...
    async function onSubmit(e: FormEvent) {
        ...
        try {
            ...
            // Editar termina a tarefa: volta para o catálogo em vez de deixar a
            // tela aberta com "Alterações salvas." e obrigar a voltar na mão.
            // Cadastrar não volta — quem cadastra costuma cadastrar vários seguidos.
            if (modo === 'editar') {
                navigate('/');
                return;
            }
    ...
    return (
        <Estado carregando={carregandoFilme} erro={erroCarregar} mensagemCarregando="Carregando título...">
            <div className="px-pad mx-auto w-full max-w-[calc(1040px+var(--spacing-pad)*2)] pt-4.5">
                <div className="surface px-[clamp(20px,5vw,56px)] pt-[clamp(28px,5vw,46px)] pb-[clamp(32px,5vw,52px)]">
                    <Cabecalho modo={modo}/>
              ...
              desmarca o outro — radiogroup/aria-checked, não aria-pressed. */}
                        ...
                        <div className="mt-6.5">
                            ...
                            <p className="field-hint">
                                Não encontrou?{' '}
                                <Link to="/gerenciar" className="text-accent hover:text-accent-text-hover">
                                    Gerencie as listas
                                </Link>
                                .
                            </p>
                        ...
                        {/* <output> já é uma live region de role status; o `block` repõe o
              display que o <p> dava, senão text-center e mt não valem. */}
                        {sucesso && (
                            <output className="text-ok mt-4.5 block text-center text-sm font-bold">
